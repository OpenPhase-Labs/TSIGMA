...
|--------|--------|---------|
| **config** | `signal`, `approach`, `detector`, `controller_type`, `jurisdiction`, `region`, `corridor`, `direction_type`, `lane_type`, `movement_type`, `detection_hardware`, `event_code_definition`, `route`, `route_signal`, `route_phase`, `route_distance`, `signal_audit`, `approach_audit`, `detector_audit`, `plugin_audit` | Configuration, reference data, routes, audit |
...
| **identity** | `auth_audit_log`, `auth_user`, `api_key`, `system_setting`, `system_setting_audit` | Authentication, system config, runtime-settings audit trail |
...
-- Runtime-settings audit trail.  One row inserted by
-- PUT /api/v1/settings/{key} for every successful registry write,
-- in the same transaction as the system_setting UPSERT.  See
-- docs/operations/runtime-settings.md for the operator reference.
CREATE TABLE system_setting_audit (
    id                  BIGSERIAL PRIMARY KEY,
    key                 TEXT NOT NULL,
    old_value           TEXT,                            -- NULL on first-time writes
...
-- Composite index for per-key history queries (newest first).
CREATE INDEX idx_system_setting_audit_key
    ON system_setting_audit (key, changed_at DESC);

-- Plugin lifecycle event log (config schema).  One row written by the
-- plugin supervisor for every launch, handshake failure, health
-- transition, restart, give-up and shutdown.  See ADR-0019.
CREATE TABLE plugin_audit (
    id                  BIGSERIAL PRIMARY KEY,
    plugin_name         TEXT NOT NULL,
    event_type          TEXT NOT NULL,                   -- PluginAuditEvent value
    process_model       TEXT NOT NULL,                   -- child | external | cron
    detail              TEXT,                            -- diagnostic, e.g. the refused handshake
    changed_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- Composite index for per-plugin history queries (newest first).
CREATE INDEX idx_plugin_audit_plugin
    ON plugin_audit (plugin_name, changed_at DESC);
```
...
| **Warm** | TimescaleDB (compressed, 10-15x) | Fast | `TSIGMA_STORAGE_COLD_AFTER_DAYS` |
| **Cold** | Parquet via `parquet_fdw` (On-Prem only) | Seconds | `TSIGMA_STORAGE_COLD_ENABLED`, `TSIGMA_STORAGE_COLD_AFTER_DAYS`, `TSIGMA_STORAGE_COLD_PATH` |
