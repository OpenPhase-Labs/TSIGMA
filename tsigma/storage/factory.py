...
def get_tile_storage_backend() -> StorageBackend:
    """Create a tile-cache storage backend rooted at tile_storage_path.
    ...
    if backend_type == "s3":
        ...
        logger.info("Creating S3 tile storage backend (bucket=%s)", settings.storage_s3_bucket)
        return S3Backend(
            bucket=settings.storage_s3_bucket,
            region=settings.storage_s3_region,
            endpoint_url=settings.storage_s3_endpoint or None,
    ...
    logger.info("Creating filesystem tile storage backend (path=%s)", settings.tile_storage_path)
    return FilesystemBackend(base_path=settings.tile_storage_path)
